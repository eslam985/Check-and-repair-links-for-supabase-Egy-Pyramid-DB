...
async def check_voe(client, url, link_id, server_name):
    async with sem: # الانتظار في الطابور
        try:
            ...
            if clean_url.endswith('/download'): clean_url = clean_url[:-9]
            file_code = clean_url.split('/')[-1][:12]
            ...
            if data.get('success'):
                ...
                if status == "200": return link_id, "valid", None, server_name, url
                if status == "404": return link_id, "broken", "API: Deleted", server_name, url
...
async def run_watcher(limit=40):
    ...
    try:
        ...
        for link_id, status, error, server_name, url in results:
            ...
            print(f"{icon} ID {link_id:<5} | {server_name:<10} | {status:<10} | URL: {url}")
